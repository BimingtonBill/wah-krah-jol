...
pub const CONVERTER_SCHEMA_VERSION: u32 = 19;
...
impl ConversionManifest {
    pub fn load(path: &Path) -> Result<Self> {
        ...
        if matches!(manifest.schema_version, 12..=18) && CONVERTER_SCHEMA_VERSION == 19 {
            // Schemas 13-19 change only NIF publication (15: vertex alpha is not blend, blend
            ...
            // made consistent between its label, its URI and its file) and the world
...
mod tests {
    ...
    fn manifests_written_before_pruned_reference_tracking_still_load() {
        ...
        // Schema 19 keys only, as written before `pruned_texture_references` existed. The
    ...
    fn recent_schema_migrations_reuse_only_unchanged_asset_kinds() {
        for schema_version in [12, 13, 14, 15, 16, 17, 18] {
            let directory = tempfile::tempdir().unwrap();
            let path = directory.path().join("conversion-manifest.json");
