...
impl ConversionManifest {
    pub fn load(path: &Path) -> Result<Self> {
        ...
        if matches!(manifest.schema_version, 12..=14) && CONVERTER_SCHEMA_VERSION == 15 {
            ...
            // normalization; schema 15 adds the world database's `statics`
            // entries for six more renderable base types. Preserve verified
            // archive ingestion, textures, and scripts, but force every GLB
            // plus the always-rebuilt world database and cell cache through
            // the new contracts.
